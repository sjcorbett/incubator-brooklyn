...
import org.apache.brooklyn.camp.brooklyn.VanillaSoftwareProcessYamlTest;
import org.apache.brooklyn.camp.brooklyn.catalog.CatalogOsgiVersionMoreEntityTest;
...
public class VanillaSoftwareProcessTest extends BrooklynRestApiLauncherTestFixture {
    ...
    public void testVanillaSoftwareProcessCanUseResourcesInBundles() throws Exception {
        ...
        final String catalogItemUrl = "classpath:/" + VanillaSoftwareProcessYamlTest.class.getPackage().getName().replace('.', '/')
                + "/vanilla-software-process-with-resource.yaml";
        final String catalogYaml = ResourceUtils.create(CatalogOsgiVersionMoreEntityTest.class)
                .getResourceAsString(catalogItemUrl);

        final URI webConsoleUri = URI.create(getBaseUri());
